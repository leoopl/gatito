...
export default function Item({name, price, description}: Props) {
    ...
    return(
        <>
            <TouchableOpacity onPress={()=>{setExpand(!expand); setAmout(1)}} style={styles.informacao}>
                <Text style={styles.nome}>{name}</Text>
                <Text style={styles.descricao}>{description}</Text>
                <Text style={styles.preco}>{Intl.NumberFormat('pt-BR', {style: 'currency', currency: 'BRL'}).format(price)}</Text>
            </TouchableOpacity>
            {expand &&
                <View style={styles.carrinho}>
                    <View>
                        <View style={styles.valor}>
                            ...
                            <Camp styles={styles.quantidade} amout={amout} setAmout={setAmout}/>
                        </View>
                        <View style={styles.valor}>
                            <Text style={styles.descricao}>Preço Total: </Text>
                            <Text style={styles.preco}>{Intl.NumberFormat('pt-BR', {style: 'currency', currency: 'BRL'}).format(finalPrice)}</Text>
                        </View>
                    </View>
